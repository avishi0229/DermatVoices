...
def extract_video_frame(video_filepath):
    ...
    try:
        ...
        raise ValueError("FFmpeg is required to analyze a video upload.") from error
    except subprocess.CalledProcessError as error:
        frame_path.unlink(missing_ok=True)
        raise ValueError("The uploaded video could not be read.") from error
...
def brain_of_the_doctor(patient_text, image_filepath=None, video_filepath=None):
    ...
    # Groq vision accepts images, so use the first video frame when no image was
    # uploaded and keep the rest of the request unchanged.
    temporary_frame = None
    visual_filepath = image_filepath
    if not visual_filepath:
        temporary_frame = extract_video_frame(video_filepath)
        visual_filepath = temporary_frame
